    ...
    class TextureCacheCubemap : TextureCache
    {
            ...
            : base(cacheName, sliceSize)
        {
            // if (HDRenderPipeline.isReady)
                m_BlitCubemapFaceMaterial = CoreUtils.CreateEngineMaterial("Hidden/BlitCubeTextureFace");
        ...
        override protected bool TransferToSlice(CommandBuffer cmd, int sliceIndex, Texture[] textureArray)
        {
            ...
            else
            {
                ...
                for (int texIDx = 0; texIDx < textureArray.Length; ++texIDx)
                {
                    if (mismatch)
                    {
                        m_BlitCubemapFaceProperties.SetTexture(ShaderPropertyId.inputTex, textureArray[texIDx]);
                        m_BlitCubemapFaceProperties.SetFloat(ShaderPropertyId.LOD, 0);
                        for (int f = 0; f < 6; f++)
                        {
                            m_BlitCubemapFaceProperties.SetFloat(ShaderPropertyId.faceIndex, (float)f);
